...
local STATION_ID = "";
local STATION_PW = ""; //Note that you must only use alphanumerics in your password. Http post won't work otherwise.
...
function windCorrect(direction) {
    ...
    local dir = temp[1].tointeger() + 180; 
    if(dir < 0) dir -= 360;
    return(temp[0] + "=" + dir);
}
